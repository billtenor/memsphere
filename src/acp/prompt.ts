...
export async function buildArtifactReviewerPrompt(input: {
  ...
  const rolePrompts = run.controlPlane?.actors[assignment.actorId]?.systemPrompt
    ? [run.controlPlane.actors[assignment.actorId].systemPrompt]
    : [];
  ...
  return [
    ...
    "Based on the current Artifact and its requirements, you may trace backward through Artifacts already produced in this Run; do not base your conclusion only on the current Artifact. When code is involved, you may also inspect the actual workspace code, diff, and tests.",
    "",
    ...renderReviewContract(contract),
    "",
    "## Earlier Artifacts",
    ...(submission.contextArtifacts.length
      ? submission.contextArtifacts.map((item) => `- ${item.stepId}: ${item.artifact.name}`)
      : ["- None."]),
    "",
    "## Effective permissions",
    ...permissionDescriptions,
    "These permissions apply only to this Assignment. You are a trusted engineering collaborator with workspace write access, but this task is review-only: do not edit product files, report a replacement Artifact, advance the Run, impersonate another reviewer, or change Slot bindings or policy.",
    "",
    ...
    "- Check this Assignment's status, Review Slots, permissions, and your own comments:",
    ...
    "2. Read the short earlier-Artifact index and inspect any prior Artifact relevant to the current claims. When implementation correctness is in scope, inspect the actual workspace diff, relevant source paths, and tests.",
    ...
    "6. In the final summary, explain the basis for the vote and any residual risks. An approve summary must explain why the evidence is sufficient, not only restate that validation passed.",
    "",
    "## Completion",
    "Check every assertion in the frozen Review contract before voting. For implementation claims, inspect relevant prior Artifacts or concrete code paths. If the available evidence is insufficient, report a blocking comment instead of claiming the implementation is correct. For each unmet assertion, preserve at least one concrete comment with an explicit severity that identifies the violated requirement and relevant evidence.",
