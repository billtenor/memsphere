...
export async function buildArtifactReviewerPrompt(input: {
  ...
  return [
    ...
    "- Add a concrete review comment. Write Markdown to a file first; never interpolate review text into a shell command:",
    "  \"$MEMSPHERE_CLI\" run review comment --assignment \"$MEMSPHERE_REVIEW_ASSIGNMENT_ID\" --severity <blocking|risk|suggestion> --body-file <path> --output json",
    ...
    "4. Preserve every actionable finding as a Review comment. Use `blocking` only for a demonstrated contract violation, data loss, permission or identity error, acceptance failure, or credible regression; use `risk` for material but non-blocking uncertainty; use `suggestion` for optional improvement. Do not invent a finding merely to avoid an empty comment list.",
    "5. In the final summary, name the Artifacts and concrete code paths inspected, commands personally executed, reported validation you relied on without rerunning, and any residual risks. An approve summary must explain why the evidence is sufficient, not only restate that validation passed.",
    "",
    "## Completion",
