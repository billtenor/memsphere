...
- After producing a non-schema artifact, report simple scalar values with `vibe-mem run report --run <id> --artifact <value>`.
...
- Prefer `--artifact-file` for all `markdown`, `json`, and `yaml` artifacts. Do not pass Markdown directly through a double-quoted shell argument, because backticks in inline code such as `` `vibe-mem` `` are interpreted by the shell before `vibe-mem` receives the value.
