...
class RunApplication {
  ...
  #reviewEditor(context:Json,runId:string):HTMLElement{
    ...
    const votes=document.createElement("div");votes.className="artifact-review-vote";votes.setAttribute("role","radiogroup");for(const [value,label] of [["approve",tr(this.#config,"approve")],["request_changes",tr(this.#config,"requestChanges")],["abstain",tr(this.#config,"abstain")]]){const choice=button(label,selectedVote===value?"active":"");choice.setAttribute("role","radio");choice.setAttribute("aria-checked",String(selectedVote===value));choice.onclick=()=>{for(const item of votes.querySelectorAll<HTMLButtonElement>("button")){const selected=item===choice;item.classList.toggle("active",selected);item.setAttribute("aria-checked",String(selected));}this.#voteByActor.set(actorKey,value);};votes.append(choice);}editor.append(votes);
