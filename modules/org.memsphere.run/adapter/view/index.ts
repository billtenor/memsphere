...
const zh: Readonly<Record<string, string>> = Object.freeze({
  ...
  reviewProgress: "评审进度", close: "关闭", submit: "提交评审", approve: "通过",
  ...
  record: "本轮汇总", saveDraft: "保存草稿", conflict: "评审已变化，请刷新后重试。",
  loadFailed: "加载 Run 失败", binding: "运行时评审绑定", status: "状态",
  identity: "评审身份", overallPlaceholder: "补充整体评审意见", addOpinion: "添加意见",
  ...
  pendingVote: "待投票", decisionReady: "决策票已就绪", waitingReviews: "仍在等待评审",
...
const en: Readonly<Record<string, string>> = Object.freeze({
  ...
  reviewProgress: "Review progress", close: "Close", submit: "Submit review", approve: "Approve",
  ...
  scope: "Review scope", record: "Round summary", saveDraft: "Save draft",
  conflict: "The review changed. Refresh and try again.", loadFailed: "Unable to load run",
  binding: "Runtime review bindings", status: "Status", identity: "Review identity",
  overallPlaceholder: "Add an overall review comment", addOpinion: "Add comment",
  ...
  pendingVote: "Pending vote", decisionReady: "Decision votes are ready", waitingReviews: "Waiting for reviews",
...
const styles = `
  ...
  .artifact-review-round-select{position:relative}.artifact-review-select{min-width:180px;display:flex;justify-content:space-between;gap:10px;border:1px solid var(--line);border-radius:6px;background:#fff;padding:7px 9px}.artifact-review-select-menu{position:absolute;z-index:10;top:calc(100% + 3px);left:0;min-width:100%;max-width:360px;padding:4px;border:1px solid var(--line);border-radius:7px;background:#fff;box-shadow:0 8px 24px #0002}.artifact-review-select-option{display:block;width:100%;border:0;background:transparent;text-align:left;padding:7px;border-radius:4px;white-space:nowrap}.artifact-review-select-option:hover,.artifact-review-select-option[aria-selected=true]{background:var(--soft)}.comment-card{padding:8px;margin:7px 0;border:1px solid var(--line);border-radius:6px}.artifact-review-activity{margin-top:9px;padding:9px;background:var(--soft);border-radius:7px}.artifact-review-activity-head,.artifact-review-activity-event-head{display:flex;align-items:center;justify-content:space-between;gap:8px}.artifact-review-activity-log{max-height:240px;overflow:auto}.artifact-review-activity-event{display:grid;gap:4px;padding:8px 0;border-bottom:1px solid var(--line)}.artifact-review-activity-event-title{display:block}
...
class RunApplication {
  ...
  #renderReviewPanel(runId:string,context:Json):HTMLElement{
    ...
    const mine=reviewCard("artifact-review-my-panel",tr(this.#config,"myReview"));const assignments=round?.assignments||[];const humans=assignments.filter((a:Json)=>a.actorKind!=="agent");if(humans.length)mine.append(this.#identity(context,humans,runId));
    if(historical){const badge=textBlock(tr(this.#config,"historicalRound"));badge.className="artifact-review-message";const notice=textBlock(tr(this.#config,"historyReadOnly"));notice.className="artifact-review-message warn";mine.append(badge,notice);}else if(!context.assignment)mine.append(textBlock(tr(this.#config,"noReviewRequired")));else if(context.assignment.actorKind!=="agent")mine.append(this.#reviewEditor(context,runId));
  ...
  #reviewEditor(context:Json,runId:string):HTMLElement{
    ...
    for(const comment of draft.comments||[])editor.append(commentCard(comment));
    const textarea=document.createElement("textarea");textarea.placeholder=tr(this.#config,"overallPlaceholder");const actorKey=context.assignment.actorId;textarea.value=this.#composerByActor.get(actorKey)||"";textarea.oninput=()=>this.#composerByActor.set(actorKey,textarea.value);
    const actions=document.createElement("div");actions.className="artifact-review-actions";const add=button(tr(this.#config,"addOpinion"));actions.append(add);editor.append(textarea,actions);
    const votes=document.createElement("div");votes.className="artifact-review-vote";for(const [value,label] of [["approve",tr(this.#config,"approve")],["request_changes",tr(this.#config,"requestChanges")],["abstain",tr(this.#config,"abstain")]]){const choice=document.createElement("input");choice.type="radio";choice.name="artifact-review-vote";choice.value=value;choice.checked=draft.vote===value;const row=document.createElement("label");row.append(choice,label);choice.onclick=()=>void this.#saveDraft(context,runId,{vote:value,comments:draft.comments||[]},choice);votes.append(row);}editor.append(votes);
    add.onclick=()=>{const body=textarea.value.trim();if(!body)return;void this.#saveDraft(context,runId,{vote:draft.vote||null,comments:[...(draft.comments||[]),{id:`comment-${Date.now()}`,body,severity:"risk"}]},add,textarea);};
  ...
  #renderMaterial(context:Json):HTMLElement{
    ...
    if(material?.commentable){const children=[...content.children] as HTMLElement[];if(children.length){children.forEach((child,index)=>{const anchor=`markdown:${child.tagName.toLowerCase()}:${index}`;child.classList.add("artifact-review-target");child.dataset.anchor=anchor;const plus=button("");plus.className="inline-plus";plus.setAttribute("aria-label",this.#config.locale?.startsWith("en")?"Add inline comment":"添加行内意见");plus.onclick=()=>this.#openInlineEditor(child,context,anchor);child.append(plus);});}else{content.classList.add("artifact-review-target");content.dataset.anchor="artifact:root";}}
    frame.append(content);wrap.append(frame);return wrap;
  }
  #openInlineEditor(target:HTMLElement,context:Json,anchor:string):void{
    target.querySelector(".inline-comment-editor")?.remove();const editor=document.createElement("div");editor.className="inline-comment-editor";const input=document.createElement("textarea");input.placeholder=this.#config.locale?.startsWith("en")?"How should this change?":"这里应该如何修改？";const save=button(tr(this.#config,"addOpinion"));save.onclick=()=>{const body=input.value.trim();if(!body)return;const draft=context.assignment?.draft||{vote:null,comments:[]};const comment={id:`comment-${Date.now()}`,body,severity:"risk",anchor:{submissionId:context.submission.id,sourceHash:context.submission.digest,target:anchor,location:anchor,context:String(target.textContent||"").replace("+","").trim().slice(0,500)}};void this.#saveDraft(context,this.#detail?.id||"",{vote:draft.vote||null,comments:[...(draft.comments||[]),comment]},save,input);};editor.append(input,save);target.append(editor);input.focus();
  }
  ...
  async #saveDraft(context:Json,runId:string,draft:Json,control:HTMLButtonElement|HTMLInputElement,composer?:HTMLTextAreaElement):Promise<void>{
    control.disabled=true;const payload:{expectedRevision:number;vote?:string;comments:Json[]}={expectedRevision:context.review.round.revision,comments:draft.comments};if(draft.vote)payload.vote=draft.vote;
    try{let response=await fetch(this.#assignmentUrl(context,"draft"),{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(payload),signal:this.#controller.signal});if(response.status===409){const latest=await this.#request(`/api/runs/${encodeURIComponent(runId)}/artifact-reviews/${encodeURIComponent(context.review.id)}/rounds/${encodeURIComponent(this.#reviewRoundId)}?actor_id=${encodeURIComponent(context.assignment.actorId)}`);response=await fetch(this.#assignmentUrl(latest,"draft"),{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({...payload,expectedRevision:latest.review.round.revision}),signal:this.#controller.signal});}if(!response.ok)throw new Error(await response.text());this.#reviewContext=await response.json();if(composer){composer.value="";this.#composerByActor.delete(context.assignment.actorId);}this.#renderReview(runId,context.review.id);}catch(error){const message=document.createElement("div");message.className="artifact-review-message warn";message.textContent=error instanceof Error?error.message:String(error);control.closest(".artifact-review-comment,.inline-comment-editor")?.prepend(message);}finally{control.disabled=false;}
  }
  #confirmSubmit(context:Json,runId:string):void{
    const confirmDialog=document.createElement("dialog");confirmDialog.className="artifact-review-dialog";const body=document.createElement("div");body.className="artifact-review-dialog-body";const title=document.createElement("h3");title.textContent=tr(this.#config,"submit");const cancel=button(tr(this.#config,"close"));const submit=button(tr(this.#config,"submit"),"primary");body.append(title,cancel,submit);confirmDialog.append(body);this.#portal.append(confirmDialog);cancel.onclick=()=>confirmDialog.close();submit.onclick=()=>void this.#withButton(submit,async()=>{const latest=this.#reviewContext||context;const response=await fetch(this.#assignmentUrl(latest,"submit"),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({expectedRevision:latest.review.round.revision}),signal:this.#controller.signal});if(!response.ok)throw new Error(await response.text());this.#reviewContext=await response.json();confirmDialog.close();await this.#refresh();this.#renderReview(runId,context.review.id);});confirmDialog.addEventListener("close",()=>confirmDialog.remove(),{once:true});confirmDialog.showModal();
  }
  ...
  #opinionBody(opinion:Json,includeComments=true):HTMLElement{const wrap=document.createElement("div");wrap.className="artifact-review-opinion";if(opinion.renderedSummary){const summary=document.createElement("div");summary.innerHTML=opinion.renderedSummary;wrap.append(summary);}else if(opinion.summary){wrap.append(textBlock(opinion.summary));}if(includeComments)for(const comment of opinion.comments||[]){const card=commentCard(comment);const severity=document.createElement("span");severity.className="artifact-review-comment-severity";severity.textContent=comment.severity||"comment";card.prepend(severity);wrap.append(card);}return wrap;}
...
function commentCard(comment:Json):HTMLElement{const card=document.createElement("div");card.className="comment-card";const body=document.createElement("span");body.textContent=comment.body||"";card.append(body);if(comment.anchor){const locate=button("定位");locate.onclick=()=>{const anchor=typeof comment.anchor==="string"?comment.anchor:comment.anchor.target;const target=document.querySelector(`[data-anchor="${CSS.escape(anchor||"")}"]`);if(target instanceof HTMLElement){target.scrollIntoView({block:"center"});target.classList.add("artifact-review-target-located");setTimeout(()=>target.classList.remove("artifact-review-target-located"),1800);}};card.append(locate);}return card;}
function chooser(className:string,label:string,current:string,items:Array<{id:string;label:string;selected:boolean}>,onSelect:(id:string)=>void|Promise<void>,role:"combobox"|"button"="combobox"):HTMLElement{const wrap=document.createElement("div");wrap.className=`artifact-review-round-select ${className}`;const trigger=document.createElement("button");trigger.type="button";trigger.className="artifact-review-select artifact-review-select-trigger";trigger.setAttribute("role",role);trigger.setAttribute("aria-label",label);trigger.setAttribute("aria-haspopup","listbox");trigger.setAttribute("aria-expanded","false");trigger.append(document.createTextNode(current),document.createTextNode("⌄"));const menu=document.createElement("div");menu.className="artifact-review-select-menu";menu.setAttribute("role","listbox");menu.setAttribute("aria-label",label);menu.hidden=true;const setOpen=(open:boolean)=>{menu.hidden=!open;trigger.setAttribute("aria-expanded",String(open));};for(const item of items){const option=document.createElement("button");option.type="button";option.className="artifact-review-select-option";option.dataset.actorId=className.includes("actor")?item.id:undefined;option.dataset.roundId=className.includes("round-select")?item.id:undefined;option.setAttribute("role","option");option.setAttribute("aria-selected",String(item.selected));option.textContent=item.label;option.onclick=()=>{setOpen(false);void onSelect(item.id);};menu.append(option);}trigger.onclick=()=>setOpen(menu.hidden);wrap.addEventListener("focusout",()=>setTimeout(()=>{if(!wrap.contains(document.activeElement))setOpen(false);},0));wrap.append(trigger,menu);return wrap;}
