...
const zh: Readonly<Record<string, string>> = Object.freeze({
  ...
  identity: "评审身份", overallPlaceholder: "补充整体评审意见", addOpinion: "添加意见", commentSeverity: "意见分类",
  severityBlocking: "阻塞问题", severityRisk: "风险", severitySuggestion: "建议",
...
const en: Readonly<Record<string, string>> = Object.freeze({
  ...
  overallPlaceholder: "Add an overall review comment", addOpinion: "Add comment", commentSeverity: "Comment severity",
...
class RunApplication {
  ...
  async #saveDraft(context:Json,runId:string,draft:Json,control:HTMLButtonElement|HTMLInputElement,composer?:HTMLTextAreaElement,rerender=true):Promise<void>{
    control.disabled=true;let active:Json=this.#reviewContext?.assignment?.actorId===context.assignment.actorId?this.#reviewContext!:context;const payload:{expectedRevision:number;vote?:string;comments:Json[]}={expectedRevision:active.review.round.revision,comments:draft.comments};if(draft.vote)payload.vote=draft.vote;
    try{let response=await fetch(this.#assignmentUrl(active,"draft"),{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify(payload),signal:this.#controller.signal});if(response.status===409){active=await this.#request(`/api/runs/${encodeURIComponent(runId)}/artifact-reviews/${encodeURIComponent(context.review.id)}/rounds/${encodeURIComponent(this.#reviewRoundId)}?actor_id=${encodeURIComponent(context.assignment.actorId)}`);response=await fetch(this.#assignmentUrl(active,"draft"),{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({...payload,expectedRevision:active.review.round.revision}),signal:this.#controller.signal});}if(!response.ok)throw new Error(await response.text());this.#reviewContext=await response.json();if(composer){composer.value="";this.#composerByActor.delete(context.assignment.actorId);}if(rerender)this.#renderReview(runId,context.review.id);}catch(error){const message=document.createElement("div");message.className="artifact-review-message warn";message.textContent=error instanceof Error?error.message:String(error);control.closest(".artifact-review-comment,.inline-comment-editor")?.prepend(message);}finally{control.disabled=false;}
  }
