...
test("init installs reserved memory into the scope root without importing it into user memory", async () => {
  await withTempDir(async (dir) => {
    ...
    assert(items.some((item) => item.path === "concepts/memsphere-reserved-memory.yaml"));
...
test("reserved memory install does not overwrite existing files", async () => {
  await withTempDir(async (dir) => {
    ...
    const target = join(reservedMemoryRoot(scopeRoot), "concepts", "memsphere-reserved-memory.yaml");
...
test("importReservedMemory copies reserved memory into the user memory root", async () => {
  await withTempDir(async (dir) => {
    ...
    const importedPath = await importReservedMemory(scopeRoot, memoryRoot, "concepts/memsphere-reserved-memory.yaml");
    assert.equal(importedPath, join(memoryRoot, "concepts", "memsphere-reserved-memory.yaml"));

    const files = await readAllMemoryFiles(memoryRoot, "concepts");
    assert.equal(files.length, 1);
    assert.equal(files[0].entity.names[0], "memsphere reserved memory");

    const items = await listReservedMemories(scopeRoot, memoryRoot);
    assert.equal(items.find((item) => item.path === "concepts/memsphere-reserved-memory.yaml")?.imported, true);
  });
});
...
test("importReservedMemory does not overwrite existing user memory", async () => {
  await withTempDir(async (dir) => {
    ...
    await writeFile(
      join(memoryRoot, "concepts", "memsphere-reserved-memory.yaml"),
      "!concept\nnames:\n  - memsphere reserved memory\ndefines:\n  - User-owned memory wins.\n"
    );

    await assert.rejects(
      importReservedMemory(scopeRoot, memoryRoot, "concepts/memsphere-reserved-memory.yaml"),
      /memory already exists/
    );

    assert.match(
      await readFile(join(memoryRoot, "concepts", "memsphere-reserved-memory.yaml"), "utf8"),
      /User-owned memory wins/
    );
  });
});
