...
- page load generation 贯穿异步 route application 和 route guard 所有权，并使新导航失效旧 Artifact Review context 请求，避免延迟 history 请求覆盖最新页面或提前释放新导航的 URL 同步保护。
- 手动切换 Memory、Task 或 Settings 时立即推进 page load generation，避免未完成的首屏路由在稍后把用户选择覆盖回旧页面。
- 响应式布局测试改为等待 CSS 过渡达到目标宽度，消除慢速 macOS runner 上固定延时带来的误报。
...
- PR 次轮 CI 暴露的首屏手动切页竞态已补回归；相关定向测试 76/76、全量测试 390 项 0 失败（1 项平台条件跳过）。
