...
- CLI 新增 `memsphere run abandon`；View 新增二次确认、独立 abandoned 分组与只读状态；废弃不自动归档，done/abandoned 均可另行归档并恢复原终态。
- 双语 Prompt、README、System Memory、Skill 和当前 Project Memory 已同步；同时修正开发规范中对已移除 `memsphere-review` Memory 的陈旧引用。
- 验证通过：`npm run typecheck`、`npm run build`、`npm test`（409 passed、1 Windows-only skipped）、Project 与 reserved-memory 的 `memsphere validate`、Embedded ChangeSet validate，以及真实 Playwright CLI 废弃后独立归档流程。
