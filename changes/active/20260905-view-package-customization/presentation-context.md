...
- 受控动作：snapshot 上的 `refresh()`、`openMemory(reference)`。创建 Memory 仍保留在官方页面，不向替换页开放写动作。
...
- 只读数据：`reference`、`kind`、冻结的 `entity` 与 `memory` 摘要。
...
- 受控动作：snapshot 上的 `refresh()`、`openRun(id)`。启动 Run 是写操作，仍通过官方流程/CLI 完成。
...
- 只读数据：`runId`、`artifactId`、冻结的 `artifact`、`event` 与 `run` 摘要。
