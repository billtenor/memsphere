...
- 匿名 Schema 必须至少包含 `format`、`asserts` 或 `fields` 等有效结构信息。
- 匿名结构不进入全局记忆名称索引。
...
`Schema.fields` 中的字符串表示只有字段名的最简字段；需要定义、断言、格式或子字段时，使用带 `!schema` tag 的详细字段。作为 field 使用的 Schema 必须具有非空 `names`。
...
`section`、`field`、`table`、`list`、`template` 等 Schema format，是内置 `!schema` 结构体的表达模式。
