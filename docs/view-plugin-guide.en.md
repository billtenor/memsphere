...
A View Plugin can be installed as a trusted local “interface extension package” without entering `builtinModuleCatalog` or rebuilding Memsphere. See `examples/view-packages/custom-view-showcase`. Settings → Appearance & Themes is organized as Extension package installation, Theme configuration, and Interface configuration. A package is the install, sharing, and upgrade unit, while its themes, pages, components, and styles can be selected independently. “Apply all” only fills those selections, which remain editable. The page has one Save action, and all configuration is stored in Home and applied to every Project. Legacy `/settings/packages` and `/settings/composition` links open the same page.
