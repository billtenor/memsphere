...
A View Plugin can be installed as a trusted local “interface extension package” without entering `builtinModuleCatalog` or rebuilding Memsphere. See `examples/view-packages/dsh-custom-view`. Settings → Appearance & Themes is organized as Extension package installation, Theme configuration, and Interface configuration. A package is the install, sharing, and upgrade unit, while its themes, pages, components, and styles can be selected independently. “Apply all” only fills those selections, which remain editable. The page has one Save action; it validates both scopes before persisting Home and Project configuration. Legacy `/settings/packages` and `/settings/composition` links open the same page.
...
Theme is global: light/dark/system mode and the selected extension-package theme apply to every Project. Legacy Project Theme fields still parse for configuration compatibility but no longer affect presentation, and Settings exposes no Project-level theme choice. Normal Settings surfaces show only saved, unsaved, or restart-required state—not revision, digest, or running/disk snapshots.
